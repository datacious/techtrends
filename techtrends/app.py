...
def post(post_id):
    ...
    if post is None:
      logger.info(f'404 not found for post {post["id"]}.')
      return render_template('404.html'), 404
    else:
      logger.info(f'Article "{post["title"]}" is retrieved')
...
def create():
    if request.method == 'POST':
        ...
        if not title:
            ...
            connection.commit()
            connection.close()
            logger.info(f'An article with title "{title}" is created.')
...
if __name__ == "__main__":
    ...
    # Get logger and set level and handler
